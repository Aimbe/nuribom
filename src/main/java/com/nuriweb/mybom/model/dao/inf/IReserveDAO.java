...
public interface IReserveDAO {
	...
	// 모든 예약정보를 조회할 수 있다. - 어드민 15개
		List<ReserveVO> selectAllReserve(int ofset,int limit);
		// 7가지 동시검색 리스트 리턴  //관리자 - 15개
		List<ReserveVO> getListSearchReserve(int Id, String Nickname,
				String CenterName, String day, int time, int visit, int review, int ofset, int limit);
		// 한 멤버의 예약리스트를 가져올수 있다. -3개
			List<ReserveVO> getAllListForMember(int userId , int ofset, int limit);
			
			
	int checkAllReserveCount(); // 전체 조회, 검색 조회, + 페이지, 날자범위...
	int checkAllReserveMemberCount(int mbId); // 전체 조회, 검색 조회, + 페이지, 날자범위...
