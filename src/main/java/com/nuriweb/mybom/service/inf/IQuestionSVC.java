...
//	1.전체 질문글 리스트를 페이지네이션 적용해서 조회할 수 있다.
	List<QuestionVO> selectAllQuestion(int page,int pgSize);
	
	
//  1-3.관리자가 답변예정인 글을 전체조회할 수 있다.
	List<QuestionVO> selectAllWaitingQuestion();
	List<QuestionVO> selectAllWaitingQuestion(int page,int pgSize);
	...
	//답변 예정 게시글 체크
	int checkAllQnACountByWaitingReply();
	
//  pageNumber 체크..
	int checkMaxpageNumber(int pgSize);
	int checkMaxpageNumberByUser(int pgSize,int mbId);
	int checkMaxpageNumberByWaiting(int pgSize);
	
	
//	1-1.특정 회원의 질문글을 전체조회할 수 있다.
	List<QuestionVO> selectAllQuestionByUser(int mbId,int page,int pgSize);
...
//  5-1.상단 검색 필드를 사용하여 해당 회원이 쓴 질문글을 제목으로 검색할 수 있다.
	...
	//다수개 삭제 처리용 
	boolean deleteMyQnA(List<Integer> qnaList);
