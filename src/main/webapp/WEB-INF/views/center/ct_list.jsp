...
<script type="text/javascript">
	...
	$(document).ready(function () {
	     ...
	     } else if( tag == 'socialRelationship') {
	    	 $('#sr').attr('selected','selected');
	     } else if( tag == 'couple') {
	    	 $('#c').attr('selected','selected');
	     } else if(tag == 'family'){
	    	 $('#fm').attr('selected','selected');
	     } else if(tag == 'study'){
	    	 $('#st').attr('selected','selected');
	     } else if(tag == 'company'){
	    	 $('#cp').attr('selected','selected');
	     } else if(tag == 'career'){
	    	 $('#cr').attr('selected','selected');
	     } else if(tag == 'job'){
	    	 $('#job').attr('selected','selected');
	     } else if(tag == 'childcare'){
	    	 $('#chc').attr('selected','selected');
	     } else if(tag == 'overseas'){
	    	 $('#os').attr('selected','selected');
	     } else if(tag == 'addicted'){
	    	 $('#add').attr('selected','selected');
	     } else if(tag == 'eatingDisorder'){
	    	 $('#do').attr('selected','selected');
	     } else if(tag == 'sexLife'){
	    	 $('#sl').attr('selected','selected');
	     } else if(tag == 'sexualMinorities'){
	    	 $('#sm').attr('selected','selected');
	     } else if(tag == 'emotionalControl'){
	    	 $('#ec').attr('selected','selected');
	     }  else if(tag == 'selfHarm'){
...
<!--                 <button type="submit" id="applicate1">적용</button> -->
                ...
                <div id="tag_wrapper">
                  <select name="sel_tag" id="center_tag">
                    <option value="all" id="all" class = "selected">모든고민</option>
                    <option value="friend"  id="fr" >친구</option>
                    <option value="married"  id="mar" >부부</option>
                    <option value="socialRelationship"  id="sr" >대외관계</option>
                    <option value="couple" id="c"  >연인</option>
                    <option value="family"  id="fm" >가족</option>
                    <option value="study"  id="st" >학업</option>
                    <option value="company"  id="cp" >직장</option>
                    <option value="career"  id="cr" >진로</option>
                    <option value="job"  id="job" >취업</option>
                    <option value="childcare" id="chc"  >육아</option>
                    <option value="overseas"  id="os" >해외생활</option>
                    <option value="addicted" id="add"  >중독</option>
                    <option value="eatingDisorder"  id="do" >섭식장애</option>
                    <option value="sexLife"  id="sl" >성생활</option>
                    <option value="sexualMinorities"  id="sm" >성소수자</option>
                    <option value="emotionalControl"  id="ec" >감정조절</option>
                    <option value="selfHarm"  id="sh" >자해</option>
                    <option value="suicide"  id="sc" >자살</option>
                  </select>
<!--                   <button type="submit" id="applicate1">적용</button> -->
                </div>
                  <!-- 필터 -->
...
<!--                     <button type="submit" id="applicate1">적용</button> -->
	...
	<c:if test="${empty ctList}">
		<div id="no_result"><i class="fas fa-compass fa-2x"></i>&nbsp; 검색 결과가 없습니다!</div>
	</c:if>
