...
<link href=" ${pageContext.request.contextPath}/resources/css/member/myReply.css" rel="stylesheet" type="text/css" />
<link href=" ${pageContext.request.contextPath}/resources/css/member/myQnA.css" rel="stylesheet" type="text/css" />
</head>
<body>
	 ...
	 <div class="container submain-content-box">
 	 <%@include file="../common/member/_my_category.jsp" %>
          ...
          <div class="real-content">
            ...
            <div class="user-top">
                ...
                <div class="user-search-box">
                  <h6 class="user-mini-title">QnA게시판</h6>
                  <form method="get">
		 	...
		 	<!-- 자유게시판 글 테이블 -->
		 	<c:if test="${empty qnaSize || qnaSize == 0}">
		 		<div class="noBoard">
				<h4> 작성하신 QnA가 없습니다.</h4>
				
			</div>
			</c:if>
			<c:if test="${qnaSize > 0}">
		 	 <%@include file="../common/member/_my_QnA_list.jsp" %>
		 	<!-- 글작성 이동 -->
		 	<div class="go-user-board-register">
              ...
              onclick="location.href='${pageContext.request.contextPath}/question_new_edit_form.bom'">글쓰기</button>
              </div>
		...
		<c:forEach begin="1" end="${maxPg}" step="1" 
			...
			.contextPath}/member_qna.bom?pg=${vs.current}'">${vs.current}</button>
			</c:if>
		...
		<c:if test="${pn < maxPg}">
			<button type="button" class="page-btn"
			onclick = "location.href = '${pageContext.request
			.contextPath}/member_qna.bom?pg=${pn+1}'"> <i class="fas fa-chevron-right fa-1x page-icon"></i></button>
		</c:if>	
            </div>	
...
$('.delete').click(function(){
     ...
     if(delVal == true){
     	...
     	for (var i=0; i< check_count; i++) {
     	 ...
     	 location.href=
     		 "${pageContext.request.contextPath}/question_delete.bom?qnaId="+checkDelete+"&mbId="+'${mbId}';
